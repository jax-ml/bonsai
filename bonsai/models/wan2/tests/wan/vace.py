...
class WanVace(WanT2V):
    def __init__(
        ...
        self.vid_proc = VaceVideoProcessor(
            downsample=tuple([x * y for x, y in zip(config.vae_stride, self.patch_size)]),
            min_area=720 * 1280,
            max_area=720 * 1280,
        ...
        with amp.autocast(dtype=self.param_dtype), torch.no_grad(), no_sync():
            ...
            for _, t in enumerate(tqdm(timesteps)):
                ...
                noise_pred_uncond = self.model(
                    latent_model_input, t=timestep, vace_context=z, vace_context_scale=context_scale, **arg_null
                )[0]
...
class WanVaceMP(WanVace):
    def __init__(self, config, checkpoint_dir, use_usp=False, ulysses_size=None, ring_size=None):
        self.config = config
    ...
    def dynamic_load(self):
        ...
        context = mp.spawn(
            self.mp_worker,
            nprocs=gpu_infer,
            args=(gpu_infer, pmi_rank, pmi_world_size, in_q_list, out_q, initialized_events, self),
            join=False,
        )
    ...
    def mp_worker(self, gpu, gpu_infer, pmi_rank, pmi_world_size, in_q_list, out_q, initialized_events, work_env):
        try:
            ...
            vae = WanVAE(vae_pth=os.path.join(self.checkpoint_dir, self.config.vae_checkpoint), device=gpu)
            ...
            while True:
                ...
                with amp.autocast(dtype=param_dtype), torch.no_grad(), no_sync():
                    if sample_solver == "unipc":
                        ...
                        sample_scheduler.set_timesteps(sampling_steps, device=gpu, shift=shift)
                        timesteps = sample_scheduler.timesteps
                    elif sample_solver == "dpm++":
                        sample_scheduler = FlowDPMSolverMultistepScheduler(
                            num_train_timesteps=num_train_timesteps, shift=1, use_dynamic_shifting=False
                        ...
                        sampling_sigmas = get_sampling_sigmas(sampling_steps, shift)
                        timesteps, _ = retrieve_timesteps(sample_scheduler, device=gpu, sigmas=sampling_sigmas)
                    else:
                        raise NotImplementedError("Unsupported solver.")
