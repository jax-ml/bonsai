    ...
    def __call__(self, prompt, system_prompt=None, tar_lang="zh", image=None, seed=-1, *args, **kwargs):
        if system_prompt is None:
            system_prompt = self.decide_system_prompt(
                tar_lang=tar_lang, multi_images_input=isinstance(image, (list, tuple)) and len(image) > 1
            )
        if seed < 0:
...
class QwenPromptExpander(PromptExpander):
    ...
    def extend_with_img(
        ...
        **kwargs,
        ...
        role_content = [{"type": "text", "text": prompt}, *[{"image": image_path} for image_path in image]]
