...
class WanVAEDecoder(nnx.Module):
    ...
    def __init__(self, *, rngs: nnx.Rngs):
        # Hardcoded denormalization constants (from reference implementation)
        # These are NOT learned parameters, but fixed constants
        mean = jnp.array(
            [
                -0.7571,
                -0.7089,
        ...
        # Reshape to [1, 1, 1, 1, 16] for broadcasting (channel-last format)
        self.latent_mean = mean.reshape(1, 1, 1, 1, 16)
        self.latent_std = std.reshape(1, 1, 1, 1, 16)
    ...
    def decode(self, latents: Array) -> Array:
        ...
        """
        # Step 1: Denormalize
        z = latents * self.latent_std + self.latent_mean
