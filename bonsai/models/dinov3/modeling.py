...
class DINOv3ViTEmbeddings(nnx.Module):
    def __init__(self, config: DINOv3ViTConfig, rngs: nnx.Rngs):
        super().__init__()
        self.config = config
        ...
        self.patch_embeddings = nnx.Conv(
            ...
            kernel_size=make_2tuple(config.patch_size),
            strides=make_2tuple(config.patch_size),
            rngs = rngs
        )
...
class Dinov3ViTAttention(nnx.Module):
    def __init__(self, config: DINOv3ViTConfig, rngs: nnx.Rngs):
        ...
        self.q_proj = nnx.Linear(in_features=config.hidden_size, out_features=config.hidden_size, use_bias=config.query_bias, rngs=rngs)
        self.k_proj = nnx.Linear(in_features=config.hidden_size, out_features=config.hidden_size, use_bias=config.key_bias, rngs=rngs)
        self.v_proj = nnx.Linear(in_features=config.hidden_size, out_features=config.hidden_size, use_bias=config.value_bias, rngs=rngs)
        self.o_proj = nnx.Linear(in_features=config.hidden_size, out_features=config.hidden_size, use_bias=config.proj_bias, rngs=rngs)
...
class Dinov3ViTModel(nnx.Module):
    def __init__(self, config: DINOv3ViTConfig, rngs: nnx.Rngs):
        ...
        self.layer = nnx.List([
            Dinov3ViTLayer(config, rngs=rngs) for _ in range(config.num_hidden_layers)
        ])
