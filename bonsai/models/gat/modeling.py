...
class GATLayer(nnx.Module):
    def __init__(
        ...
        self.a = nnx.Param(
            nnx.initializers.glorot_uniform()(
                rngs.params(), (num_heads, 2 * out_features), jnp.float32
            )
        )
    ...
    def __call__(self, h: jax.Array, adj: jax.Array) -> jax.Array:
        # h: (N, in_features)
        # adj: (N, N)
        N = h.shape[0]
...
class GAT(nnx.Module):
    ...
    def __call__(self, x: jax.Array, adj: jax.Array, training: bool = False) -> jax.Array:
        h = x
