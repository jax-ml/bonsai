...
def load_model(name: str, device: str = "cpu", download_root: str = None):
    """Load a real Whisper model with actual weights."""
    ...
    dims = model_configs[name]
    rngs = nnx.Rngs(0)
    model = Whisper(dims, rngs=rngs)
    
    # Load real weights from PyTorch Whisper
    try:
        import whisper
        print(f"🔄 Loading real {name} Whisper weights...")
        ...
        print(f"✅ Real {name} Whisper weights loaded successfully!")
        
    except ImportError:
        print(f"⚠️  PyTorch Whisper not available, using random weights")
    except Exception as e:
        print(f"⚠️  Failed to load real weights: {e}, using random weights")
