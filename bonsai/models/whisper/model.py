...
class MultiHeadAttention(nnx.Module):
    ...
    def qkv_attention(self, q: jnp.ndarray, k: jnp.ndarray, v: jnp.ndarray, mask: Optional[jnp.ndarray] = None):
        ...
        n_kv_ctx = k.shape[1]  # k and v can have different sequence length (cross-attention)
        scale = (n_state // self.n_head) ** -0.25
        
        # Reshape q, k, v for multi-head attention
        q = q.reshape(n_batch, n_ctx, self.n_head, -1).transpose(0, 2, 1, 3)  # (batch, head, q_len, head_dim)
        k = k.reshape(n_batch, n_kv_ctx, self.n_head, -1).transpose(0, 2, 3, 1)  # (batch, head, head_dim, kv_len)
        v = v.reshape(n_batch, n_kv_ctx, self.n_head, -1).transpose(0, 2, 1, 3)  # (batch, head, kv_len, head_dim)

        # Compute attention scores: (batch, head, q_len, kv_len)
        qk = jnp.matmul(q, k) * scale
        ...
        # Reshape back to (batch, q_len, state)
        wv = wv.transpose(0, 2, 1, 3).reshape(n_batch, n_ctx, n_state)
